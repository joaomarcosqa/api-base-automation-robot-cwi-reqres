...
Criar Usuário com Sucesso
    ...
    ${response}=    Post Request    reqres    /users    json=${body}    headers=${HEADERS_JSON}
...
Listar Usuários com Sucesso
    ${response}=    Get Request    reqres    /users    headers=${HEADERS_JSON}
    Should Be Equal As Integers    ${response.status_code}    ${SuccessStatusCode200}
    ${json}=    To Json    ${response.content}
    Validate No Empty Fields    ${json}

Editar Usuário com Sucesso
    [Setup]    Run Keyword If    '${CREATED_USER_ID}' == ''    Criar Usuário com Sucesso
    ${update}=    Create Dictionary    name=John Doe Updated    job=Senior Developer
    ${response}=    Put Request    reqres    /users/${CREATED_USER_ID}    json=${update}    headers=${HEADERS_JSON}
    Should Be Equal As Integers    ${response.status_code}    ${SuccessStatusCode200}
    ${json}=    To Json    ${response.content}
    Validate No Empty Fields    ${json}

Buscar Usuário por ID
    ${response}=    Get Request    reqres    /users/1    headers=${HEADERS_JSON}
    Should Be Equal As Integers    ${response.status_code}    ${SuccessStatusCode200}
    ${json}=    To Json    ${response.content}
...
Excluir Usuário
    ${response}=    Delete Request    reqres    /users/2    headers=${HEADERS_JSON}
...
Erro 401 por Falta de API Key
    ${body}=    Create Dictionary    name=${USER_NAME}    job=${USER_JOB}
    ${response}=    Post Request    reqres    /users    json=${body}    headers=${HEADERS_NO_AUTH}
    Should Be Equal As Integers    ${response.status_code}    ${FailStatusCode401}
    ${json}=    To Json    ${response.content}
